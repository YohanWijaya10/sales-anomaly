...
import { getDatesBetween, getLastCompleteMonthRange, getMonthRangeForDate } from "@/lib/utils/date";
...
import {
  ...
  generateMonthlyFallbackInsight,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const period = date ? getMonthRangeForDate(date) : getLastCompleteMonthRange();
    const { from, to } = period;
    const prevMonthAnchor = shiftDate(from, -1);
    const prevPeriod = getMonthRangeForDate(prevMonthAnchor);
    ...
    const perDay = await Promise.all(
      dates.map(async (dateStr) => {
        const metrics = await computeDailyMetricsForDate(dateStr);
        const redFlags = await getAllRedFlagsForDate(dateStr, metrics.salesmen_metrics);
        return { date: dateStr, metrics, redFlags };
      }),
    );

    const perDayPrev = await Promise.all(
      prevDates.map(async (dateStr) => {
        const metrics = await computeDailyMetricsForDate(dateStr);
    ...
    const insight =
      process.env.DEEPSEEK_API_KEY
        ? await generateMonthlyInsight(input)
        : generateMonthlyFallbackInsight(input);
    ...
    console.error("Error insight bulanan:", error);
    return NextResponse.json(
